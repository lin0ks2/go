...
(function () {
  ...
  // ────────────────────────────────────────────────────────────
  // ПУЛ ОТВЛЕКАЮЩИХ ДЛЯ «МОИ ОШИБКИ» — БЕЗ ПУСТЫХ КНОПОК
  function getMistakesDistractorPool(currentWord) {
    try {
      const uniq = new Map();
      const push = (w) => {
        if (!w || !w.id || String(w.id) === String(currentWord.id)) return;
        // у варианта должна быть видимая подпись хотя бы на одном из путей
        const hasLabel = ((App.settings.lang === 'ru') ? (w.ru || w.uk) : (w.uk || w.ru)) || w.word || w.translation || w.meaning;
        if (!hasLabel) return;
        const key = String(w.id) + '::' + (w.word || w.ru || w.uk || '');
        if (!uniq.has(key)) uniq.set(key, w);
      };

      // 1) из полных карточек «МОИ ОШИБКИ»
      if (App.Mistakes && typeof App.Mistakes.deck === 'function') {
        const arr = App.Mistakes.deck() || [];
        for (let i = 0; i < arr.length; i++) push(arr[i]);
      }

      // 2) из исходного словаря текущего слова
      let srcKey = null;
      try { srcKey = (App.Mistakes && App.Mistakes.sourceKeyFor) ? App.Mistakes.sourceKeyFor(currentWord.id) : null; } catch (_) {}
      if (srcKey) {
        const srcDeck = App.Decks.resolveDeckByKey(srcKey) || [];
        for (let i = 0; i < srcDeck.length; i++) push(srcDeck[i]);
      }

      // 3) при нехватке — добор из всех словарей (без фанатизма)
      if (uniq.size < 20) {
  ...
  function addToMistakesOnFailure(word) {
    ...
    try {
      ...
      let sourceKey;
      if (active === 'mistakes') {
        // тренируем сам «МОш»: сохраняем по исходному словарю
        sourceKey =
          (word && (word._mistakeSourceKey || word._favoriteSourceKey)) ||
          (App.Mistakes && typeof App.Mistakes.sourceKeyFor === 'function' ? App.Mistakes.sourceKeyFor(String(word.id)) : null) ||
          'mistakes';
      } else {
        sourceKey = active;
      }
